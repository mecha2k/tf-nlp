...
encoded_result = tokenizer.encode(
    "전율을 일으키는 영화. 다시 보고싶은 영화", max_length=max_seq_len, truncation=True
)
...
def convert_examples_to_features(examples, labels, max_seq_len, tokenizer):
    ...
    for example, label in tqdm(zip(examples, labels), total=len(examples)):
        input_id = tokenizer.encode(
            example, max_length=max_seq_len, padding="max_length", truncation=True
        )
...
def sentiment_predict(new_sentence):
    input_id = tokenizer.encode(
        new_sentence, max_length=max_seq_len, padding="max_length", truncation=True
    )
