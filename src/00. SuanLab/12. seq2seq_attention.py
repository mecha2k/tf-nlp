...
print(encoder_in_data.shape)
print(decoder_in_data.shape)
...
decoder_outputs = decoder_softmax(concat)
...
s_ = tf.concat([encoder_state_input_h[:, tf.newaxis, :], decoder_outputs[:, :-1, :]], axis=1)
context_vector = Attention()([s_, encoder_outputs])
decoder_concat = tf.concat([decoder_outputs, context_vector], axis=-1)
decoder_outputs = decoder_softmax(decoder_concat)

decoder_model = Model(
    inputs=[decoder_inputs, encoder_state_input_h, encoder_outputs] + decoder_state_inputs,
...
def decode_sequences(input_seq):
    ...
    while not stop:
        output_tokens, h, c = decoder_model.predict(
            [target_seq, states_value[0], encoder_outputs] + states_value
        )
