...
def prepare_data():
    ...
    return preprocessed_documents, unpreprocessed_corpus, vocab, tp, training_dataset
...
def combined_topic_modeling():
    ...
    ctm = CombinedTM(
        ...
        n_components=50,
        num_epochs=20,
        num_data_loader_workers=0,  # set it to 0 if you are using Windows
    )
    ctm.fit(training_dataset)

    # 토픽들 학습 후에는 토픽 모델이 선정한 토픽들을 보려면 아래의 메소드를 사용합니다.
    # get_topic_lists
    # 해당 메소드에는 각 토픽마다 몇 개의 단어를 보고 싶은지에 해당하는 파라미터를 넣어즐 수 있습니다. 여기서는 5개를 선택했습니다.
    # 아래의 토픽들은 위키피디아(일반적인 주제)으로부터 얻은 토픽을 보여줍니다.
    # 우리는 영어 문서로 학습하였으므로 각 토픽에 해당하는 단어들도 영어 단어들입니다.
    ...
    # vis.display(ctm_pd)
    vis.save_html(ctm_pd, "images/comb_topic.html")
    ...
    # 차후 사용을 위해 모델 저장하기
    ctm.save(models_dir="../data/ctm_eng")
...
def load_ctm_models():
    ...
    ctm.load(
        "../data/ctm_eng/contextualized_topic_model_nc_50_tpm_0.0_tpv_0.98_hs_prodLDA_ac_(100, 100)_do_softplus_lr_0.2_mo_0.002_rp_0.99",
        epoch=19,
    )
    print(ctm.get_topic_lists(5))
...
if __name__ == "__main__":
    ...
    preprocessed_documents, unpreprocessed_corpus, vocab, tp, training_dataset = prepare_data()
