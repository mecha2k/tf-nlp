...
# 문맥을 반영한 토픽 모델(Contextualized Topic Models, CTM)이란 무엇일까요? CTM은 BERT 임베딩의 표현력과 토픽 모델의 비지도 학습의
# 능력을 결합하여 문서에서 주제를 가져오는 토픽 모델의 일종입니다.
#
...
#
# - 영어가 아닌 다른 언어로 작업하고 싶으시다면: [여기를 클릭!
#   (https://contextualized-topic-models.readthedocs.io/en/latest/language.html#language-specific)
# - 토픽 모델에서 좋은 결과가 나오지 않는다면: [여기를 클릭!]
#   (https://contextualized-topic-models.readthedocs.io/en/latest/faq.html#i-am-getting-very-poor-results-what-can-i-do)
# - 여러분의 임베딩을 사용하고 싶다면: [여기를 클릭!]
#   (https://contextualized-topic-models.readthedocs.io/en/latest/faq.html#can-i-load-my-own-embeddings)
#
# GPU를 사용하세요
...
#
# 데이터
# 학습을 위한 데이터가 필요합니다. 여기서는 하나의 라인(line)에 하나의 문서로 구성된 파일이 필요한데요. 우선, 여러분들의 데이터가 없다면
# 여기서 준비한 파일로 실습을 해봅시다.
...
def combined_topic_modeling():
    ...
    # 여기서 전처리 된 텍스트를 사용하는 이유는 무엇일까요? Bag of Words를 구축하려면 특수문자가 없는 텍스트가 필요하고, 모든 단어를
    # 사용하는 것보다는 빈번한 단어들만 사용하는 것이 좋습니다.
